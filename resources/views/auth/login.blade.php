...
<head>
  ...
  <title>Login — {{ \App\Models\Setting::get('desa_nama','Desa') }}</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;background:linear-gradient(135deg,color-mix(in srgb,{{ $hijau }} 18%,#f6f7fb) 0%,#f6f7fb 55%,color-mix(in srgb,{{ $hijau }} 7%,#f6f7fb) 100%);z-index:0}
    body::after{content:'';position:fixed;top:-30%;right:-10%;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,color-mix(in srgb,{{ $hijau }} 12%,transparent),transparent 70%);z-index:0}
    ...
    .logo-icon{width:68px;height:68px;background:{{ $hijau }};border-radius:16px;display:inline-flex;align-items:center;justify-content:center;font-size:30px;box-shadow:0 4px 16px color-mix(in srgb,{{ $hijau }} 40%,transparent)}
    ...
    input[type=email]:focus,input[type=password]:focus{border-color:{{ $hijau }};box-shadow:0 0 0 3px color-mix(in srgb,{{ $hijau }} 14%,transparent)}
    .btn{width:100%;padding:13px;background:{{ $hijau }};color:#fff;border:none;border-radius:9px;font-family:inherit;font-size:14.5px;font-weight:700;cursor:pointer;transition:opacity .15s,transform .1s;margin-top:4px}
    .btn:hover{opacity:.9}
    .btn:active{transform:scale(.99)}
    .remember{display:flex;align-items:center;gap:9px;font-size:13px;color:#6b7280;margin-bottom:22px;cursor:pointer}
    .remember input{width:16px;height:16px;accent-color:{{ $hijau }};cursor:pointer}
...
<body>
  <div class="box">
    <div class="logo-wrap">
      ...
      @endif
      <div class="desa-name">{{ \App\Models\Setting::get('desa_nama','Desa') }}</div>
    ...
    <div class="card">
      ...
      @endif
      @if($errors->any())
        <div class="al al-err"><i class="bi bi-exclamation-circle"></i> {{ $errors->first() }}</div>
      @endif
      ...
      <form method="POST" action="{{ route('admin.login.post') }}">
        ...
        <div class="fg">
          <label>Email</label>
          <input type="email" name="email" value="{{ old('email') }}" placeholder="admin@desa.id" required autofocus>
        </div>
        <div class="fg">
    ...
    <div class="back"><a href="{{ route('home') }}">← Kembali ke Layanan Warga</a></div>
  </div>
</body>
</html>
