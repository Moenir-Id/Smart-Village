...
<head>
  ...
  <title>@yield('title','Layanan Surat') — {{ \App\Models\Setting::get('desa_nama','Desa') }}</title>
  @php
...
<body class="@yield('body_class','')">
  <header class="hdr">
    <div class="hdr-in">
      @if($logoUrl)
        <img src="{{ $logoUrl }}" class="hdr-logo" alt="Logo">
      @else
        <div class="hdr-icon">🏛️</div>
      @endif
      <div>
        <div class="hdr-name">{{ \App\Models\Setting::get('desa_nama','Desa') }}</div>
        <div class="hdr-sub">Layanan Surat Mandiri Digital</div>
      </div>
      <a href="{{ route('admin.login') }}" class="hdr-pill" style="text-decoration:none;color:inherit"><i class="bi bi-person" style="margin-right:5px"></i>Login Staff</a>
    </div>
  </header>
  <main>
    <div class="pw">@yield('content')</div>
  </main>
  <footer>
    {{ \App\Models\Setting::get('desa_nama','Desa') }} · Layanan Surat Mandiri Digital
