...
<head>
  ...
  <title>@yield('title','Panel') — {{ \App\Models\Setting::get('desa_nama','Desa') }}</title>
...
<aside class="sb" id="sidebar">
  <div class="sb-top">
    @if($logoUrl)
      <img src="{{ $logoUrl }}" class="sb-logo" alt="Logo">
    @else
      <div class="sb-logobox">🏛️</div>
    @endif
    <div>
      <div class="sb-name">{{ \App\Models\Setting::get('desa_nama','Desa') }}</div>
  ...
  <nav class="sb-nav">
    ...
    <a href="{{ route('admin.antrean.index') }}" class="sb-a {{ request()->routeIs('admin.antrean.*') ? 'on' : '' }}">
      <i class="bi bi-inbox-fill"></i> Antrean Surat
      @php $pending = \App\Models\Permohonan::where('status','pending')->count() @endphp
      @if($pending > 0)<span class="sb-badge">{{ $pending }}</span>@endif
    </a>
    <a href="{{ url('admin/laporan') }}" class="sb-a {{ request()->is('admin/laporan*') ? 'on' : '' }}">
      <i class="bi bi-bar-chart-fill"></i> Laporan
    </a>

    @role('admin,petugas')
    <div class="sb-sec">Konten</div>
    <a href="{{ route('admin.jenis-surat.index') }}" class="sb-a {{ request()->routeIs('admin.jenis-surat.*') ? 'on' : '' }}">
      <i class="bi bi-collection-fill"></i> Jenis Surat
    </a>
    <a href="{{ route('admin.template.index') }}" class="sb-a {{ request()->routeIs('admin.template.*') ? 'on' : '' }}">
    ...
    <a href="{{ route('admin.whatsapp.index') }}" class="sb-a {{ request()->routeIs('admin.whatsapp.*') ? 'on' : '' }}">
      <i class="bi bi-whatsapp"></i> WhatsApp
      @if(\App\Models\Setting::get('wa_notif_aktif','0') == '1')<span class="sb-dot"></span>@endif
    </a>
    <a href="{{ route('admin.users.index') }}" class="sb-a {{ request()->routeIs('admin.users.*') ? 'on' : '' }}">
      <i class="bi bi-people-fill"></i> Pengguna
    </a>
    <a href="{{ route('admin.pengaturan.index') }}" class="sb-a {{ request()->routeIs('admin.pengaturan.*') ? 'on' : '' }}">
      <i class="bi bi-gear-fill"></i> Pengaturan
    </a>
    @endrole

    @role('admin,viewer')
    <a href="{{ url('admin/backup') }}" class="sb-a {{ request()->is('admin/backup*') ? 'on' : '' }}">
      <i class="bi bi-shield-fill-check"></i> Backup &amp; Restore
    </a>
    <a href="{{ route('admin.audit-log.index') }}" class="sb-a {{ request()->routeIs('admin.audit-log.*') ? 'on' : '' }}">
...
<div class="main">
  <header class="topbar">
    ...
    <div class="tb-right">
      <a href="{{ route('home') }}" class="btn btn-ol btn-sm" target="_blank">
  ...
  <div class="content">
    @if(session('success'))
      <div class="al al-ok"><i class="bi bi-check-circle-fill"></i><span>{{ session('success') }}</span></div>
    @endif
    @if(session('error'))
      <div class="al al-err"><i class="bi bi-exclamation-circle-fill"></i><span>{{ session('error') }}</span></div>
    @endif
    @yield('content')
  </div>
</div>
...
{{-- ═══ MOBILE BOTTOM NAV ═══ --}}
@php $pendingMob = \App\Models\Permohonan::where('status','pending')->count(); @endphp

<nav class="mob-nav" id="mobNav" aria-label="Navigasi utama">

  <a href="{{ route('admin.dashboard') }}"
     class="mn-item {{ request()->routeIs('admin.dashboard') ? 'on' : '' }}">
    <i class="bi bi-grid-1x2-fill mn-ico"></i>
    <span class="mn-lbl">Dashboard</span>
  </a>

  <a href="{{ route('admin.antrean.index') }}"
     class="mn-item {{ request()->routeIs('admin.antrean.*') ? 'on' : '' }}">
    <i class="bi bi-inbox-fill mn-ico"></i>
    @if($pendingMob > 0)
      <span class="mn-badge">{{ $pendingMob > 9 ? '9+' : $pendingMob }}</span>
    @endif
    <span class="mn-lbl">Antrean</span>
  </a>

  <a href="{{ url('admin/laporan') }}"
     class="mn-item {{ request()->is('admin/laporan*') ? 'on' : '' }}">
...
<div class="mob-drawer" id="mobDrawer" role="dialog" aria-modal="true">
  ...
  {{-- User info --}}
  <div class="md-head">
    <div class="md-av">{{ strtoupper(substr(auth()->user()->name,0,2)) }}</div>
    <div>
      <div class="md-uname">{{ auth()->user()->name }}</div>
      <div class="md-urole">{{ auth()->user()->role }}</div>
    </div>
  </div>
  ...
  <a href="{{ route('admin.jenis-surat.index') }}"
     class="md-a {{ request()->routeIs('admin.jenis-surat.*') ? 'on' : '' }}">
    <i class="bi bi-collection-fill"></i> Jenis Surat
  </a>
  <a href="{{ route('admin.template.index') }}"
     class="md-a {{ request()->routeIs('admin.template.*') ? 'on' : '' }}">
    <i class="bi bi-file-earmark-word-fill"></i> Template Surat
  </a>
  @endrole
  ...
  <a href="{{ route('admin.whatsapp.index') }}"
     class="md-a {{ request()->routeIs('admin.whatsapp.*') ? 'on' : '' }}">
    <i class="bi bi-whatsapp"></i> WhatsApp
    @if(\App\Models\Setting::get('wa_notif_aktif','0') == '1')<span class="md-a-dot"></span>@endif
  </a>
  <a href="{{ route('admin.users.index') }}"
     class="md-a {{ request()->routeIs('admin.users.*') ? 'on' : '' }}">
    <i class="bi bi-people-fill"></i> Pengguna
  </a>
  <a href="{{ route('admin.pengaturan.index') }}"
     class="md-a {{ request()->routeIs('admin.pengaturan.*') ? 'on' : '' }}">
    <i class="bi bi-gear-fill"></i> Pengaturan
  </a>
  @endrole
  ...
  <a href="{{ url('admin/backup') }}"
     class="md-a {{ request()->is('admin/backup*') ? 'on' : '' }}">
    <i class="bi bi-shield-fill-check"></i> Backup &amp; Restore
  </a>
  <a href="{{ route('admin.audit-log.index') }}"
     class="md-a {{ request()->routeIs('admin.audit-log.*') ? 'on' : '' }}">
    <i class="bi bi-journal-text"></i> Audit Log
  </a>
  @endrole

  <a href="{{ route('home') }}" target="_blank" class="md-a">
    <i class="bi bi-globe2"></i> Halaman Warga
  </a>

  <form method="POST" action="{{ route('admin.logout') }}" style="margin:0 0 4px">@csrf
