...
<div class="wa-grid">
  ...
  <div class="wa-left">
    ...
    <div class="card">
      ...
      <div class="cb" style="text-align:center;padding:20px">
        ...
        <div id="state-connected" style="display:none">
          ...
          <form method="POST" action="{{ route('admin.whatsapp.disconnect') }}" style="margin-top:18px"
    ...
    <div class="card">
      ...
      <div class="cb" style="padding:16px">
        <form method="POST" action="{{ route('admin.whatsapp.pengaturan') }}">
          @csrf
  ...
  <div class="card">
    ...
    <div class="cb" style="padding:16px">
      ...
      <form method="POST" action="{{ route('admin.whatsapp.template') }}">
        @csrf
        <div style="display:flex;flex-direction:column;gap:20px">
          @foreach(['diproses'=>['Sedang Diproses','#1e40af','#dbeafe'],'selesai'=>['Selesai','#166534','#dcfce7'],'ditolak'=>['Ditolak','#991b1b','#fee2e2']] as $status => [$label,$clr,$bg])
          <div>
            <div style="display:flex;align-items:center;gap:9px;margin-bottom:8px">
              <span style="padding:3px 11px;border-radius:20px;font-size:11.5px;font-weight:700;background:{{ $bg }};color:{{ $clr }}">{{ $label }}</span>
              <span class="fl" style="margin:0;font-weight:500;color:var(--subtle)">Template pesan</span>
            </div>
            <textarea name="wa_template_{{ $status }}" rows="{{ $status==='ditolak'?5:4 }}" class="fc"
              style="font-family:'DM Mono',monospace;font-size:12.5px;resize:vertical"
              placeholder="Kosongkan untuk pakai template bawaan">{{ $s['wa_template_'.$status]??'' }}</textarea>
            @if($status==='ditolak')
...
async function pollStatus() {
  try {
    ...
    if (data.ready) {
      const full = await fetch('{{ route("admin.whatsapp.status") }}', {
        headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
      });
